...
# recuperation des features triées par importance à partir du model xgboost le plus performant
...
# on sélectionne les 160 features les plus importantes :
# 1) on commence par un one-encoding : on obtient une matrice sparse
# 2) on transforme cette matrice en data.frame afin de pouvoir sélectionner les colonnes par leur nom
# 3) on sélectionne les features/colonnes dans  le data.frame
nb.features <- 160
training2<- as.data.frame(as.matrix(sparse.model.matrix(outcome~.-1, data = training)))
training2 <- training2  %>%  select(df_importance$importance_matrix.Feature[1:nb.features])
training2$outcome <- training$outcome
colnames(training2) <- make.names(names(training2))
training2 <- data.table(training2) 

dtrain <- xgb.DMatrix(data = sparse.model.matrix(outcome~.-1, data = training2),
                      label=as.numeric(training2$outcome)-1)



testing$outcome=0
testing2 <- as.data.frame(as.matrix(sparse.model.matrix(outcome~.-1, data = testing)))
testing2 <- testing2  %>%  select(df_importance$importance_matrix.Feature[1:nb.features])
colnames(testing2) <- make.names(names(testing2))
testing2 <- data.table(testing2)
testing2$outcome=0
dtest <- sparse.model.matrix(outcome~.-1, data = testing2)


colsamples_bytree <- seq(from=0.35,to=1,by=0.1)
max_depths <- 6:7
    ...
    for (max_depth in max_depths){
        for (min_child_weight in min_child_weights){
            ...
            if(error < best_params$error){
                ...
                print(best_params)
                write.csv(best_params,"best_params_feature_selection.csv")
                # Pass in our hyperparameteres and train the model 
                xgb.save(xgbmodel, "best_xgb_model_faeture_selection")
